...
class SessionModel extends EventTarget {
    ...
    async evaluateSessionStatus(){
        ...
        if( lochash != '' ){
            ...
            if( 'u' in urlks){
                var pdub = ( 'pw' in urlks ) ? urlks['pw'] : urlks.u ; 
                this.loginVault( {domain: urlks.u, pw:pdub, fn: 'login_vault', action: 'login_vault'} )
            }
            ...
            // OR autoguest login to be authenticated after 
            if( 'f' in urlks ){
                //await sleep(1500);
                

                var eventObj={}
                eventObj['uuid'] = urlks.f;   
                eventObj['requested_uuid'] = urlks.f;   
                eventObj['fn'] = 'file_launch';
    ...
    async pushNavState( obj ){
        ...
        switch( this.session.state_change ){
            ...
            case 'file_launch':
                ...
                // Load a static JSON file from disk, await the result, and set its contents to a variable
                let jsonData = null;
                try {
                    var idpath =  this.session.selected_object['uuid']; 
                    const response = await fetch('/data/miccco/'+idpath+'.json');
                    if (!response.ok) throw new Error('Network response was not ok');
                    jsonData = await response.json();
                } catch (err) {
                    console.error('Failed to load static JSON:', err);
                }
                var kobj=jsonData;  
                ...
                {   
                    ...
                    kobj.meta['origin']='int';
                    kobj.meta['foreign_id']='uuid';
                    kobj.meta['foreign_a']='a';
                    kobj.meta['foreign_b']='b';
                    // this.matrix.mergeMap( actobj.dat );
                    this.matrix.incomingDiffFromKeyCache( kobj );
    ...
    async pushMessage( obj ){
        ...
        if( obj.fn == 'detailEvent'){
            ...
            fabObj.uuid='generic01';
            fabObj.method ='deepreport';
            fabObj.params = aiObj;
            fabric.mergeIntent( fabObj );

            // should detail event come through here //
            this.dispatchEvent(new CustomEvent('detailEvent',{ detail: {model:this , params:obj} } ) ) 
            ...
            case 'generic':
                switch( e.detail.method ){
                    ...
                    case 'deepreport':
                        var response = {}
                        var okthen = 0;
                        this.dispatchEvent( new CustomEvent('deepreportEvent',  { detail:e.detail } ) ) 
